...
export class DynamicFormComponent implements OnInit {
    ...
    public ngOnInit(): void {
        ...
        this.formConfig.form.forEach(gr => {
            ...
            if (gr.enableWhen && !this.checkRules(gr.enableWhen, this.model)) { fields.forEach(f => f.disabled = true); }
        });
    ...
    public checkRules(enableWhen: EnableWhenConfig, data): boolean {
        let enabled = true;
        ...
        if (enableWhen.rules.length === 1) {
            ...
            enabled = rule.equals.indexOf(data[rule.field]) > -1;
        }
        ...
        if (enableWhen.rules.length > 1) {
            ...
            if (enableWhen.type === ConditionType.Or) {
                ...
                enableWhen.rules.forEach(rule => {
                    enabled = enabled || rule.equals.indexOf(data[rule.field]) > -1;
                });
            }
